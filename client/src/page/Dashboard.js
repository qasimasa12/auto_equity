...
export default function Dashboard() {
  ...
  const [selects,setSelects] = useState("Select Month")
  const [fetchTableData,setFetchTableData] = useState([])
  
  useEffect(() => {
    fetch("http://16.171.38.251/api/userData", {
      method: "POST",
      crossDomain: true,
      headers: {
  ...
  useEffect(() => {
    if(selects !== "Select Month" ){
      ...
      settable_dashbord2(filteredTable);
    }else{
      settable_dashbord2(fetchTableData);
    }



  }, [selects, fetchTableData]);
  ...
  useEffect(()=>{
    ...
    const {leadsAvalible,setLeadsAvalible} = useContext(Context);

    function handleClick (ix){
        setLeadsAvalible(table_dashbord2[ix])
              ...
              <div className="col-4 col-md-3 rowell set_end">
                ...
                <Form.Select
                  ...
                  className="size_select fondsixw ms-2"
                    value={selects} onChange={e=>{handleChange(e)}}
                >
                    ...
                    <Table bordered hover className="table with_table">
                      ...
                      <tbody>
                        {table_dashbord2.map((table_dashbord2,ix) => (
                          <tr key={ix} onClick={()=>handleClick(ix)}>
                            <td>{ix+1}</td>
